...
function buscarKpisRisco(fkUniversidade) {
    ...
    var instrucaoSql = `
        SELECT
            SUM(CASE WHEN ir.score <= 40 THEN 1 ELSE 0 END) AS baixo,
            SUM(CASE WHEN ir.score BETWEEN 41 AND 69 THEN 1 ELSE 0 END) AS medio,
...
function buscarAlunos(fkUniversidade) {
    ...
    var instrucaoSql = `
        SELECT
            ...
            ir.score,
            CASE
                WHEN ir.score <= 40 THEN 'Baixo'
                WHEN ir.score BETWEEN 41 AND 69 THEN 'Médio'
                ELSE 'Alto'
            END AS risco
        FROM Aluno a
        JOIN Historico h ON h.fkAluno = a.RA
